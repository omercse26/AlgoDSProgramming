...
class CommandProcessor(threading.Thread):
    def __init__(self, command=('echo hello', None), logdir = None, cwd=None, daemonprocess=False, waitcmdprompt=False, exceptionqueue=None):
        
        self.exceptionqueue = exceptionqueue
        
        if hasattr(command, '__call__') == True:
            self.command = command            
            super(CommandProcessor, self).__init__(target=self.command)
            ...
            super(CommandProcessor, self).__init__(target=t)                    
...
def runCommandsInSequence(cmdlist, logdir, cwd=None, waitcmdprompt=False):
    for cmd in cmdlist:
        p = CommandProcessor(command=cmd, cwd=cwd, daemonprocess=False, waitcmdprompt=waitcmdprompt, logdir=logdir)
        p.start()
        p.wait()
        
def runCommandsInParallel(cmdlist, logdir, cwd=None, daemon = False, waitcmdprompt=False):
    processlist = []
    errormsgs = ""
    for cmd in cmdlist:
        p = CommandProcessor(command=cmd, cwd=cwd, daemonprocess=daemon, waitcmdprompt=waitcmdprompt, logdir=logdir)        
        p.start()
...
def runMethodsInParallel(*methods):
    ...
    for method in methods:
        p = CommandProcessor(command=method, exceptionqueue=exception_queue)
        p.start()
        threads.append(p)
